...
flags["color_picker_anim_speed"] = 1.5
...
function library:window(properties)
	...
	local date_table = os.date("*t", real_time)

	local month_text = months[date_table.month]
	local day_text = tostring(date_table.day)
	local year_text = tostring(date_table.year)

	local watermark_text = "Intoria Paid - " .. month_text .. " " .. day_text .. " " .. year_text .. " Hood Customs Free For All   "
	-- ====================================

	local animated_text = library:animation(watermark_text)
	...
	local inline1 = library:create("Frame", {
		...
		-- ===== Анимированная обводка для watermark =====
	local wm_stroke = library:create("UIStroke", {
		Parent = inline1,
		Name = "WmAnimatedBorderStroke",
		Thickness = 2,
	...
	local wm_gradient = library:create("UIGradient", {
		Parent = wm_stroke,
		Name = "WmBorderGradient",
		Rotation = 0,
		Color = ColorSequence.new({
			ColorSequenceKeypoint.new(0.00, Color3.fromRGB(90, 90, 90)),
			ColorSequenceKeypoint.new(0.25, Color3.fromRGB(255, 255, 255)),
			ColorSequenceKeypoint.new(0.50, Color3.fromRGB(90, 90, 90)),
	...
	task.spawn(function()
		local rot = 0
		local off = 0
		while inline1.Parent do
			rot = (rot + 1.5) % 360
			off = (off + 0.005) % 1
			wm_gradient.Rotation = rot
			wm_gradient.Offset = Vector2.new(off, 0)
			task.wait()
		end
	end)
	-- ==============================================
...
local TEXT_ANIMATION_GRADIENT = library:create("UIGradient", {
    ...
    Name = "",
    Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Color3.fromRGB(90, 90, 90)),   -- серый
        ColorSequenceKeypoint.new(0.5, Color3.fromRGB(255, 255, 255)), -- белый
        ColorSequenceKeypoint.new(1, Color3.fromRGB(90, 90, 90)),   -- серый
    }),
})
	...
	library:make_resizable(inline1)
		...
		-- ============ ВОЛНИСТАЯ ОБВОДКА ВОКРУГ ОКНА ============
	local border_holder = library:create("Frame", {
		Parent = inline1,
		Name = "AnimatedBorder",
		BackgroundTransparency = 1,
		Position = UDim2.new(0, -2, 0, -2),
	...
	local border_stroke = library:create("UIStroke", {
		Parent = inline1,
		Name = "AnimatedBorderStroke",
		Thickness = 2,
		Color = Color3.fromRGB(255, 255, 255),
		ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
		LineJoinMode = Enum.LineJoinMode.Round,
	...
	-- Градиент, который будет "плыть" по периметру
	local border_gradient = library:create("UIGradient", {
		Parent = border_stroke,
		Name = "BorderGradient",
		Rotation = 0,
		Color = ColorSequence.new({
			ColorSequenceKeypoint.new(0.00, Color3.fromRGB(90, 90, 90)),    -- серый
			ColorSequenceKeypoint.new(0.25, Color3.fromRGB(255, 255, 255)), -- белый
			ColorSequenceKeypoint.new(0.50, Color3.fromRGB(90, 90, 90)),    -- серый
			ColorSequenceKeypoint.new(0.75, Color3.fromRGB(255, 255, 255)), -- белый
			ColorSequenceKeypoint.new(1.00, Color3.fromRGB(90, 90, 90)),    -- серый
		}),
	})

	-- Анимация: вращаем градиент + смещаем offset для эффекта "плывущей" волны
	task.spawn(function()
		local rot = 0
		local off = 0
		while inline1.Parent do
			...
			border_gradient.Rotation = rot
			border_gradient.Offset = Vector2.new(off, 0)
...
task.spawn(function()
    while true do
        if flags["color_picker_anim_speed"] then
            ...
            local pos = math.abs(math.sin(tick() * (flags["color_picker_anim_speed"] or 1)))
            TEXT_ANIMATION_GRADIENT.Color = ColorSequence.new({
                ColorSequenceKeypoint.new(0, Color3.fromRGB(90, 90, 90)),     -- серый
                ColorSequenceKeypoint.new(math.clamp(pos, 0.01, 0.99), Color3.fromRGB(255, 255, 255)), -- белый
                ColorSequenceKeypoint.new(1, Color3.fromRGB(90, 90, 90)),     -- серый
            })
        end
        task.wait()
...
-- keybind list
	...
	-- ===== Анимированная обводка для keybinds =====
	local kb_stroke = library:create("UIStroke", {
		Parent = inline1,
		Name = "KbAnimatedBorderStroke",
		Thickness = 2,
		Color = Color3.fromRGB(255, 255, 255),
		ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
		LineJoinMode = Enum.LineJoinMode.Round,
	...
	local kb_gradient = library:create("UIGradient", {
		Parent = kb_stroke,
		Name = "KbBorderGradient",
		Rotation = 0,
		Color = ColorSequence.new({
			ColorSequenceKeypoint.new(0.00, Color3.fromRGB(90, 90, 90)),
			ColorSequenceKeypoint.new(0.25, Color3.fromRGB(255, 255, 255)),
	...
	task.spawn(function()
		...
		while inline1.Parent do
			rot = (rot + 1.5) % 360
			off = (off + 0.005) % 1
			kb_gradient.Rotation = rot
			kb_gradient.Offset = Vector2.new(off, 0)
			task.wait()
		end
	end)
	-- =============================================
...
function library:notification(properties)
	...
	-- ================= HEADER BAR (V2 | Intoria | Hood Customs) =================
	...
	-- Hood Customs Free For All (справа, красным)
	library:create("TextLabel", {
		Parent = header,
		Name = "RightLabel",
		Font = Enum.Font.Code,
		Text = "Hood Customs Free For All",
...
function library:keybind(properties)
    ...
    if cfg.key_display then
        if cfg.active then
            cfg.key_display.set_color(Color3.fromRGB(0, 255, 100))
        else
            cfg.key_display.set_color(Color3.fromRGB(170, 170, 170))
        end
    end
